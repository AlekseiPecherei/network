...
import com.alexey.network.parser.CurrentParser;
...
public class CurrentForecast extends BaseForecast {
	...
	public org.w3c.dom.Document process(String placeId, int day) {
		try {
			CurrentParser parser = new CurrentParser();
			...
			parametersMap.put(Parcer.KEY_FORECAST_WATER_TEMP, p.get(4).text());
			parametersMap.put(Parcer.KEY_FORECAST_HEOM, p.get(5).text());
			parametersMap.put(Parcer.KEY_FORECAST_SUNRISE, p.get(6).text());
			parametersMap.put(Parcer.KEY_FORECAST_SUNSET, p.get(7).text());
			parametersMap.put(Parcer.KEY_FORECAST_DAY_DURATION, p.get(8).text());
			parametersMap.put(Parcer.KEY_FORECAST_MOON, p.get(9).text());
